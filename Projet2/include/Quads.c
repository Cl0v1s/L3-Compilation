...
int Value_get(struct Value* value, struct Stack* stack, struct EnvC3A* global, struct EnvC3A* local,  struct EnvC3A* used)
{
    int tmp;
    switch(value->type)
    {
        case 'V':
            ...
            return EnvC3A_get_value(used, (char*)value->value);
        case 'I':
        case 'B':
